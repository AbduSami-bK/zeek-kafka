...
if [[ ! -f "${DATA_PATH}"/example-traffic/exercise-traffic.pcap ]]; then
  wget https://github.com/zeek/try-zeek/raw/master/manager/static/pcaps/exercise_traffic.pcap -O "${DATA_PATH}"/example-traffic/exercise-traffic.pcap
fi
...
if [[ ! -f "${DATA_PATH}"/ssh/ssh.pcap ]]; then
  wget https://github.com/zeek/try-zeek/raw/master/manager/static/pcaps/ssh.pcap -O "${DATA_PATH}"/ssh/ssh.pcap
fi
